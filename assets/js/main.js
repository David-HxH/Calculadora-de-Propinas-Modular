document.addEventListener(`DOMContentLoaded`, () => {
    ...
    if (crearLista === true) {
        let listaDeTareas = [];
        let tarea = prompt (`Ingresa una tarea para agregarar a la lista de tareas (o presiona cancelar para salir):`)

        while (tarea !== null && tarea.trim() !== ``) {
            listaDeTareas.push(tarea.trim());
            const confirmar = confirm(`Deseas agregar otra tarea?`)
            if (confirmar === true) {
                tarea = prompt (`Ingresa una tarea para agregarar a la lista de tareas (o presiona cancelar para salir):`)
